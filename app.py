...
# Define your LAN IP range
LAN_IP_RANGE_START = '192.168.0.1'
LAN_IP_RANGE_END = '192.168.0.256'

# Convert IP addresses to integers for easy comparison
def ip_to_int(ip):
    return sum([int(x) << (8 * i) for i, x in enumerate(reversed(ip.split('.')))])

LAN_IP_RANGE_START_INT = ip_to_int(LAN_IP_RANGE_START)
LAN_IP_RANGE_END_INT = ip_to_int(LAN_IP_RANGE_END)

# Check if the IP is within the LAN IP range
def is_in_lan(ip):
    ip_int = ip_to_int(ip)
    return LAN_IP_RANGE_START_INT <= ip_int <= LAN_IP_RANGE_END_INT

@app.before_request
def restrict_to_lan():
    if not is_in_lan(request.remote_addr):
        raise Forbidden("Access Forbidden - Only LAN users are allowed to access this application.")
